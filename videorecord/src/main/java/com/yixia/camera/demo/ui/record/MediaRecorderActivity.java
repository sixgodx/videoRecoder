...
import com.yixia.view.TimerView;
import com.yixia.view.VideoRecordButton;
...
public class MediaRecorderActivity extends BaseActivity implements
        ...
        MediaRecorderBase.OnEncodeListener, VideoRecordButton.OnLongPressListener {
    ...
    private static OnFinishRecordingListener sListener;
    ...
    private CheckedTextView delete;
    private CheckedTextView mDelete;
    private CheckedTextView accept;
    private CheckedTextView mAccept;
     ...
     * @param listener
     */
    public static void startActivity(Context context, @NonNull OnFinishRecordingListener listener) {
        if (listener == null) {
            throw new NullPointerException("OnFinishRecordingListener can'n be null");
        }
        sListener = listener;
        ...
        if (resultCode == RESULT_OK) {
            switch (requestCode) {
                case 7002:
                    ...
                    sListener.onRecordFinished(path);
                    finish();
                    sListener = null;
                    break;
            }
    ...
    private void loadViews() {
        ...
        mRecordController = (VideoRecordButton) findViewById(R.id.record_controller);
        mBottomLayout = (RelativeLayout) findViewById(R.id.bottom_layout);
        mRecordLed = (CheckBox) findViewById(R.id.record_camera_led);
        mTimerView = (TimerView) findViewById(R.id.timer);
        mDelete = (CheckedTextView) findViewById(R.id.record_delete);
        mAccept = (CheckedTextView) findViewById(R.id.record_accept);
        ...
        mRecordController.setOnLongPressListener(this);
...
//        mSurfaceView.setLayoutParams(lp);
    ...
    private boolean checkCameraFocus(MotionEvent event) {
        ...
        FrameLayout.LayoutParams lp = (FrameLayout.LayoutParams) mFocusImage
                .getLayoutParams();
    ...
    private void checkBtnStatus() {
        ...
        mAccept.setEnabled(duration > RECORD_TIME_MIN);

        LinkedList<MediaObject.MediaPart> parts = mMediaObject.getMedaParts();
        for (MediaObject.MediaPart part : parts) {
            if (!part.remove) {
    ...
    /**
     * 删除所选片段
     * 如果不想显示删除的片段 直接遍历mMediaObject.getMedaParts() 删除
     */
    private void onDeleteClicked() {
        if (mMediaObject != null) {
            LinkedList<MediaObject.MediaPart> medaParts = mMediaObject.getMedaParts();
            for (int i = medaParts.size() - 1; i >=0; i--) {
                MediaObject.MediaPart part = medaParts.get(i);
                if (part != null && !part.remove) {
                    part.remove = true;
                    mTimerView.truncate(part.getDuration());//不统计删除片段的时间
                    if (mProgressView != null)
                        mProgressView.invalidate();
                    break;
                }
